...
const CATALOG_URL = `${DEFAULT_API_BASE_URL}${API_PREFIX}/catalog.json`;
const API_CACHE = "ai2m2ia-pwa-api-v2";
...
const UI_LANGUAGE_LABELS = {
  en: "English", "pt-BR": "Portuguese (Brazil)", "es-419": "Spanish (Latin America)",
  fr: "French", it: "Italian", de: "German", pl: "Polish", tr: "Turkish",
  ru: "Russian", id: "Indonesian", vi: "Vietnamese", fil: "Filipino", th: "Thai",
...
async function loadCatalog(options = {}) {
  try {
    ...
    const allBooks = state.catalog.books || [];
    state.books = allBooks.filter(book => isSafeSpokeUrl(book.spokeUrl));
...
function renderLibrary() {
  ...
  nodes.count.textContent = t().count(books.length, state.books.length);
  ...
  if (!books.length) {
    renderEmptyState(nodes.bookGrid, t().noBooks);
    return;
  }

  for (const book of books) {
    nodes.bookGrid.append(renderBookCard(book));
  }
}
...
function renderBookCard(book) {
  ...
  if (isSafeSpokeUrl(book.spokeUrl)) {
    readLink.href = book.spokeUrl;
  } else {
    readLink.removeAttribute("href");
  }
  readLink.textContent = t().read;
...
function renderFallbackCover(container, book) {
  ...
  titleEl.textContent = shortTitle(book.title);
  const author = document.createElement("span");
  author.textContent = book.author || "AI(2)M(2)IA";
  fallback.append(format, titleEl, author);
...
function applyUiStrings() {
  ...
  if (!nodes.status.textContent || nodes.status.textContent === UI_STRINGS.en.syncing) {
